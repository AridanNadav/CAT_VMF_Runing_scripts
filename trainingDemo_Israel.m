...
stackW=wWidth/3;
stackH=stackW*.75;
centerRect=[ xcenter-ceil(stackW/2) ycenter-stackH/2 xcenter+ceil(stackW/2) ycenter+stackH/2];
...
%     load('Misc\demo_go.mat');
    ...
    for i = 1:length(demoNames)
        demo_images{i} = imread([ pwd,'\stim\demo\',demoNames{i}]);
    end
    ...
    for Pos = 1:length(demoNames) %length(stop{runNum}),   % To cover all the items in one run.
        ...
        Screen('PutImage',w,demo_images{Pos},centerRect);
       % Screen('Flip',w,onsets(Pos)+runStartTime); % display images according to Onset times
        Screen('Flip',w,1+runStartTime); % display images according to Onset times
