...
baseline_fixation_dur = 1; % Need to modify based on if first few volumes are saved or not
afterrunfixation = 1;
...
stackW=wWidth/3;
stackH=stackW*.75;
leftRect=[ceil(xcenter-stackW-stackW/3) ycenter-stackH/2 ceil(xcenter-stackW/3) ycenter+stackH/2];
rightRect=[ceil(xcenter+stackW/3) ycenter-stackH/2 ceil(xcenter+stackW+stackW/3) ycenter+stackH/2];
...
if test_comp == 1
    Screen('TextSize',w, 30); % 30); % 30); % 30); % 30);
    CenterText(w,'PART 4:', white,0,-325);
    
    Screen('TextSize',w, 30); % 30); % 30); % 30); % 30); % 40);
    ...
    if numRun == 1 && (block ==1||block ==3) % If this is the first run of the first or third block, show instructions
        ...
        Screen('TextSize',w, 30); % 30); % 30); % 30); % 40);
...
while GetSecs < prebaseline+baseline_fixation_dur
    ...
    Screen('TextSize',w, 30); % 30); % 30); % 30); % 60);
    Screen(w,'Flip');
...
for trial = 1:trialsPerRun
   ...
   % StimOnset = Screen(w,'Flip', runStart+1+baseline_fixation);
