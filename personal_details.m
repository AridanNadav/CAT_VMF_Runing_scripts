...
language = questdlg('Please select your primary language:','language','English','Francais','English');
while isempty(language)
    language = questdlg('Please select your  primary language:','language','English','Francais','English');
end
...
while isempty(Age) || isempty(Age{1})
    Age = inputdlg ('Only integers between 18 and 120 are valid. Please enter your age: ','Age',1);
end
Age = cell2mat(Age);
Age = str2double(Age);
while mod(Age,1) ~= 0 || Age < 18 || Age > 120
    Age = inputdlg ('Only integers between 18 and 120 are valid. Please enter your age: ','Age',1);
    Age = cell2mat(Age);
    Age = str2double(Age);
end
...
fprintf(fid1,'%s\t%d\t%s\t%s\t%d\t%d\t%d\t%s\n', subjectID, order, timestamp,language, Gender, Age, DominantHand, Occupation);
fclose(fid1);
end
